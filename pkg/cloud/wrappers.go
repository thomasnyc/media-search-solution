...
func (q *QuotaAwareGenerativeAIModel) GenerateContent(ctx context.Context, contents []*genai.Content, outputSchema *genai.Schema) (resp *genai.GenerateContentResponse, err error) {
	// set the desired output schema, take it from
	q.GenerativeContentConfig.ResponseSchema = outputSchema
	...
	if q.RateLimit.Allow() {
		...
		if err != nil {
			...
			return q.ModelHandle.GenerateContent(errCtx, q.ModelName, contents, q.GenerativeContentConfig)
		}
		...
		return q.ModelHandle.GenerateContent(ctx, q.ModelName, contents, q.GenerativeContentConfig)
	}
}
